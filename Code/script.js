function submitForm() {
    ...
    if (isNaN(n)) {//проверка N
        alert("Неправильное значение для N");
        return false
    }
    let k = parseInt(form.elements.k.value);
    if (isNaN(k)) {//проверка k
        alert("Неправильное значение для K");
        return false
    }

    if (form.elements.m.value.length != k) {//проверка длины инф. вектора
        alert("Длина информационного вектора должна быть равна N")
        return false
    }
    let m = []
    for (let char of form.elements.m.value) {//проверка значения инф. вектора
        if (char !== '0' && char !== '1') {
            alert("Неправильное значение для информационного вектора");
            return false
        }
        m.push(+char)
    }

    if (form.elements.g.value.length > (n - k + 1)) {//проверка длины порождающего вектора
        alert("Длина порождающего вектора должна быть равна N")
        return false
    }
    let g = []
    for (let char of form.elements.g.value) {//проверка порождающего вектора
        if (char !== '0' && char !== '1') {
            alert("Неправильное значение для порождающего вектора");
            return false
        }
        g.push(+char)
    }
